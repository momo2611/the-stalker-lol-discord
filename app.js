...
//pri sv 829731681929527316
const channelId2 = '829731681929527316' //test sv
app.use('/', router);

app.listen(3000);

//discord api key
client.login('MTAwMjA5MjI5NDY0OTk0MjA2Ng.Gawq_z.1euq1TlE0v8OjThtV0C_OLTlPf02LP6MlN1TXA');
client.once('ready', () => {
    console.log("Thế Stalker is now online");
});
...
client.on('messageCreate', async (message) => {
    if (message.content.toLowerCase().startsWith('!ts')) {
        ...
        if (lastWord == "info") {
            ...
            client.channels.cache.get(channelId2).send(">>> **Name: ** ``" + info.data.name + "\n``**Level:** ``" + info.data.summonerLevel
        ...
        if (lastWord == "game") {
            ...
            if (typeof gameInfo.data.gameId === 'undefined') {
                client.channels.cache.get(channelId2).send("``" + summonerName + "`` is not in game");
            }
            else {
                ...
                client.channels.cache.get(channelId2).send(">>> **game mode:** ``" + gameInfo.data.gameMode + "\n``**game type:** ``" + gameInfo.data.gameType + "\n``**champion:** ``" + currentChampion
                    + "\n``**game time:** ``" + time + "``");
            }
        ...
        for (let f = 0; f < messageArray.length; f++) {
            if (messageArray[f] == 'champion') {
                ...
                if (champInfo.data.chestGranted == false) {
                    client.channels.cache.get(channelId2).send(">>> **champion:**`` " + championName + "\n``**mastery level:** ``" + champInfo.data.championLevel +
        ...
        if (beforeLastWord == "rank") {
            ...
            if (lastWord == 'solo') {
                ...
                if (queueType[0] == 'RANKED_SOLO_5x5') {
                    client.channels.cache.get(channelId2).send(">>> **queue:** ``" + queueType[0] + "``\n**rank:**`` " + tier[0] + " " + rank2[0] + "``\n**points:** ``" + points[0]
                        + "``\n**wins/losses:** ``" + wins[0] + "/" + losses[0] + " (" + winRate + "%)``");
                }
                else {
                    client.channels.cache.get(channelId2).send(">>> **queue:** ``" + queueType[1] + "``\n**rank:**`` " + tier[1] + " " + rank2[1] + "``\n**points:** ``" + points[1]
